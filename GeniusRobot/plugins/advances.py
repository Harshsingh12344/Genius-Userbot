# Copyright © By @GeniusBoi // @AdityaHalder
import asyncio
...
Kaal = f"**꧁ 𒈞ʛeƞɩʋs υѕєявσт𒈞꧂**\n\n"
Kaal += f"━───────╯•╰───────━\n"
Kaal += f"➠ **ᴘʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `5.0 beta`\n"
Kaal += f"➠ **ᴘʏʀᴏɢʀᴀᴍ ᴠᴇʀsɪᴏɴ** : `{__version__}`\n"
Kaal += f"➠ **ᴠᴇʀsɪᴏɴ**  : `{2.0}`\n"
Kaal += f"➠ **ᴄʜᴀɴɴᴇʟ** : [❝𝐂𝐥𝐢𝐜𝐤❞](https://t.me/adityaserver)\n"
Kaal += f"━───────╮•╭───────━\n\n"
Kaal += f"➠ **𒆜ʍǟӄɛ ʏօʊʀ օառ 𒆜:** [❝𝐂𝐥𝐢𝐜𝐤❞](https://gitHub.com/GeniusBoi/Genius-Userbot)"
...
@Client.on_message(command(["help"]) & genius_filters)
...
@Client.on_message(command(["delayspam"]) & genius_filters)
...
@Client.on_message(command(["pornspam"]) & genius_filters)
...
@Client.on_message(command(["ping"]) & genius_filters)
async def oahgfg(xspam: Client, e: Message):
      await e.reply_text(f"═══════════════════\n ꧁ 𒈞ʛeƞɩʋs υѕєявσт𒈞꧂ \n═══════════════════")


@Client.on_message(command(["broadcast"]) & genius_filters)
...
@Client.on_message(command(["hang"]) & genius_filters)
async def pornspam(xspam: Client, e: Message): 
    counts = e.command[1]
    if not counts:
        return await e.reply_text(usage)
...
@Client.on_message(command(["genius"]) & genius_filters)
async def hello(client: Client, message: Message):
    await client.send_photo(message.chat.id, ALIVE_IMAGE, caption=Kaal)


@Client.on_message(command(["join"]) & genius_filters)
...
@Client.on_message(command(["leave"]) & genius_filters)
...
@Client.on_message(command(["spam"]) & genius_filters)
...
@Client.on_message(command(["raid"]) & genius_filters)
